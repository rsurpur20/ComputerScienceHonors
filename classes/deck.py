...
class Deck:
    def __init__(self):
        ...
        for i in range(1,14):
            # a=str(Card(i, "spades"))
            self.decklist.append(Card(i, "spades"))
            self.decklist.append(Card(i, "hearts"))
            self.decklist.append(Card(i, "diamonds"))
            self.decklist.append(Card(i, "clubs"))
            self.i=self.i+1
            # so now my deck of cards is made right?
        # return decklist
    def takerandomcard(self,rank,suit):
        self.decklist.remove(Card(rank, str(suit)))
        return decklist
